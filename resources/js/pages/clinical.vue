...
const clinicalDelete = ref(null)

const deleteForm = useForm({})

const clinicalDeleteTest = (test) => {
    clinicalDelete.value = test
  showDeleteModal.value = true
}

const confirmDelete = () => {
  deleteForm.delete(`/clinical/${clinicalDelete.value}`, {
    onSuccess: () => {
      showDeleteModal.value = false
      clinicalDelete.value = null
    }
  })
}
...
const print = (test) => {
    ...
    let url = ''
    if (type === 'Fecalysis') {
        url = `/clinical/${test.id}/fecalysis-pdf`
    } else if (type === 'Urinalysis') {
        url = `/clinical/${test.id}/urinalysis-pdf`
    } else {
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        <div v-for="test in patientData.tests" :key="test.id" 
                                            ...
                                            <div class="flex items-start justify-between mb-4">
                                                ...
                                                <button 
                                                    @click="clinicalDeleteTest(test.id); showResultsSection = false"
