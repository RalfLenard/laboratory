...
<body>
    ...
    <table class="ml-100 mb-10">
        ...
        <tr>
            ...
            <td><strong>Reported:</strong>
                {{ \Carbon\Carbon::parse($serology->created_at)->format('F j, Y') }}
            </td>
    ...
    <tr>
    <td class="pl" style="padding-right: 180px;"><b>HIV 1 and 2 Screening</b></td>
    @if (!empty($serology->hiv_result))
        @php
            $hiv_res = is_array($serology->hiv_result)
                ? $serology->hiv_result
                : json_decode($serology->hiv_result, true);
        @endphp

        <td rowspan="3">
            @foreach ($hiv_res as $item)
                @php
                    $type = strtoupper($item['type']);
                    $value = isset($item['details'][0]) ? strtoupper($item['details'][0]) : 'N/A';
                    $color = in_array($value, ['POSITIVEs', 'REACTIVE']) ? 'red' : 'black';
                    $padding = in_array($value, ['POSITIVEs', 'REACTIVE']) ? '40px' : '0';
                @endphp
                 
                <div style=" padding-right: {{ $padding }}; line-height: 1.5; ">
                {{ $type }} : <b class="cap">{{ ucfirst(strtolower($value)) }}</b>
                </div>
            @endforeach
        </td>
    @endif
        ...
        <tr>
            <td class="pl">Kit use: {{ $serology->hiv_kit ?? ' ' }}</td>
            <td></td>
        </tr>

        <tr>
            <td class="pl">LOT NO. {{ $serology->hiv_lot_no ?? ' ' }}</td>
            <td></td>
        </tr>

        <tr>
            <td class="pl">Expiration Date:  {{ \Carbon\Carbon::parse($serology->hiv_expiration_date ?? now())->format('F Y') }}</td>
            <td></td>
        </tr>
    ...
    <hr>
    <p class="ml-100"><strong>Remarks:</strong> {{ $serology->hiv_remarks ?? 'N/A' }}</p>
